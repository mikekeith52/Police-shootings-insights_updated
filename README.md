...
The logistic model offers estimated coefficient interpretation. The interpretation of each coefficient value is its exponentiation minus 1. All p-values less than 0.05 can be considered statistically significant with 95% certainty.
...
We can see there is some overlap, but there are fewer statistically significant variables this go around. There are several reasons that could have happened--adding more inputs to the model could have unbiased some of the estimates, the model may have been overfit, or just the timeframe flagged some false positives last time. Furthermore, I had combined the Hispanic and Black races last time. This time, I was only interested in white vs. black. Further exploration of these model interpretations can be found [below](#key-findings).  

The accuracy of this model, when tested on the test split was 70% total accuracy. The no-information rate was 64%. This model is slightly better than simply guessing, which to me, is very interesting considering that these aren't the best inputs, nor is this the easiest question to answer.
